# Sync checklist: VetClinicSystem JO → VetClinicSystem_IQ parity
...
| 1.3.0 | Bulk Barcode Print | ✅ **DONE** | Ported from VetClinicSystem_IQ's `inventory_catalog_barcodes_generated()`/`inventory_catalog_barcodes_bulk_print()`, adapted for the one real difference: Jordan has no `barcode_source` column or manual-barcode-entry feature at all (every barcode is created via the existing `inventory_catalog_create_barcode()` route), so the picker/print routes drop the `barcode_source='generated'` filter entirely and treat any item with a non-null `barcode` as eligible. Added `GET /inventory-catalog/barcodes/generated` (JSON list feeding the picker) and `POST /inventory-catalog/barcodes/bulk-print` (re-validates every id/qty server-side — never trusts the client's list — clamps qty to 1-500, ignores unknown ids) to `app.py`; new `templates/barcode_bulk_print.html` (adapted from IQ's version to call `JsBarcode` directly per label like Jordan's existing `barcode_label.html` does, instead of IQ's `vzRenderBarcode()`/`barcode-render.js` wrapper, which Jordan doesn't have); wired a "Bulk Barcode Print" button + `#bulkBarcodeModal` picker + its JS into `templates/inventory_catalog.html` (plain `display:flex`/`none` modal toggling and `alert()` for errors, matching Jordan's existing modal/no-toast conventions, not IQ's `VZSpring`/`VZToast`); added `.barcode-sheet` print-grid CSS to `static/style.css` (Jordan had `.barcode-label-box` from the single-print page but no multi-label sheet layout yet). Live-verified end-to-end against a real local Postgres 16 (fresh `createdb`, `schema_postgres.sql`, `import_seed.py`, app started, driven with a scripted cookie-jar client over curl-equivalent HTTP calls): created 2 inventory items, generated a barcode for each via the existing create-barcode route, confirmed `GET /inventory-catalog/barcodes/generated` returned exactly those 2 items with id/name/barcode, then `POST /inventory-catalog/barcodes/bulk-print` with qty 3 and 2 rendered exactly 5 `.barcode-label-box` divs (3 for item 1, 2 for item 2); a follow-up request with qty 9999 and a bogus id confirmed server-side clamping to 500 labels and silent exclusion of the nonexistent id; also confirmed the Inventory Catalog page renders the button/modal/JS only once a barcode exists. Note for whoever reviews this commit: this session's edits landed in a working tree that also has a large, unrelated, already-in-progress INTEGER→BOOLEAN column-type migration across `schema_postgres.sql`/`app.py`/`logic.py`/`auth.py`/`setup.py`/`generate_test_data.py`/`import_seed.py` (not made by this task) — this feature's own new SQL (`active=true` for `inventory_list.active`) matches whatever that column's live type is, but the migration itself is out of scope here and wasn't authored or verified by this pass. |
