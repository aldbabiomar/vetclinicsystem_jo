...
| 1.0.0 | Negative-value guards (price/weight/unit cost), fractional BCS rejection, `distributor_delete()` explains linkage | **PORT — boarding done, weight/unit-cost/BCS/distributor_delete still open** | Added the shared `has_negative()` helper and applied it to `boarding_new()`/`boarding_edit()` (price_per_day/total) as part of the same pass as the boarding-totals fix above. The weight/unit-cost/fractional-BCS input guards and `distributor_delete()`'s linkage explanation are separate call sites not yet touched. |
